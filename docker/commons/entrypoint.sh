...
GOVPAY_CONSOLE_MIN_POOL=${GOVPAY_CONSOLE_MIN_POOL:-2}
GOVPAY_CONSOLE_MAX_POOL=${GOVPAY_CONSOLE_MAX_POOL:-5}
    ...
    export SPRING_DATASOURCE_HIKARI_MINIMUM_IDLE="${GOVPAY_CONSOLE_MIN_POOL}"
    export SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE="${GOVPAY_CONSOLE_MAX_POOL}"
...
if [ -n "${GOVPAY_CONSOLE_CLUSTER_ID}" ]; then
    export CLUSTER_ID="${GOVPAY_CONSOLE_CLUSTER_ID}"
    log_info "Cluster ID: ${GOVPAY_CONSOLE_CLUSTER_ID}"
fi
...
JVM_MEMORY_OPTS="-XX:MaxRAMPercentage=${GOVPAY_CONSOLE_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}"
[ -n "${GOVPAY_CONSOLE_JVM_INITIAL_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:InitialRAMPercentage=${GOVPAY_CONSOLE_JVM_INITIAL_RAM_PERCENTAGE}"
[ -n "${GOVPAY_CONSOLE_JVM_MIN_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MinRAMPercentage=${GOVPAY_CONSOLE_JVM_MIN_RAM_PERCENTAGE}"
[ -n "${GOVPAY_CONSOLE_JVM_MAX_METASPACE_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxMetaspaceSize=${GOVPAY_CONSOLE_JVM_MAX_METASPACE_SIZE}"
[ -n "${GOVPAY_CONSOLE_JVM_MAX_DIRECT_MEMORY_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxDirectMemorySize=${GOVPAY_CONSOLE_JVM_MAX_DIRECT_MEMORY_SIZE}"
...
log_info "Java: MaxRAMPercentage=${GOVPAY_CONSOLE_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}%"
log_info "========================================"
...
JAR_FILE=$(find /opt/govpay-console -name "*.jar" -type f | head -n 1)

if [ -z "${JAR_FILE}" ]; then
    log_error "Nessun file JAR trovato in /opt/govpay-console"
    exit 1
fi
