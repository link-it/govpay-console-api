...
/**
 * Colonne di configurazione condivise tra la tipologia di versamento globale
 * ({@link TipoVersamento}, tabella {@code tipi_versamento}) e la sua
 * specializzazione per dominio ({@link TipoVersamentoDominio}, tabella
 * {@code tipi_vers_domini}). Le due tabelle hanno gli stessi nomi di colonna per
 * questi campi; le differenze (codice/descrizione sul globale, api key e foreign
 * key sul dominio) restano sulle sottoclassi concrete.
 */
@MappedSuperclass
public abstract class AbstractTipoVersamento {
