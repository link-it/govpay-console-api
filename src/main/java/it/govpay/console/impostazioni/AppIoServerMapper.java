...
/**
 * Conversione bidirezionale tra {@link ImpostazioniAppIoServer} e le proprieta'
 * EAV del connettore ({@code connettori}), stessa infrastruttura e stessa
 * codifica dell'autenticazione di {@link ServizioGdeMapper}. In V1
 * {@code AppIOBatch extends Connettore} (stesso auth completo a 6 tipi), qui
 * riusato invece di un "solo api-key" bespoke.
 */
@Component
public class AppIoServerMapper {
    ...
    public ImpostazioniAppIoServer toDto(Map<String, String> config) {
        ImpostazioniAppIoServer dto = new ImpostazioniAppIoServer();
        ...
        dto.setTimeToLiveSecondi(parseIntOrNull(config.get(ConnettoreProprietaKeys.TIME_TO_LIVE)));
        dto.setAuth(readAuth(config));
        return dto;
    }

    public Map<String, String> toConfigMap(ImpostazioniAppIoServer dto) {
        ...
        if (dto.getTimeToLiveSecondi() != null) {
            map.put(ConnettoreProprietaKeys.TIME_TO_LIVE, String.valueOf(dto.getTimeToLiveSecondi()));
        }
