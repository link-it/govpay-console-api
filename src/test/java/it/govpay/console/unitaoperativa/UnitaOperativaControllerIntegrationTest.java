...
class UnitaOperativaControllerIntegrationTest {
    ...
    void listUngrantedDominioForNonStarOperatorReturns404() throws Exception {
        ...
        mvc.perform(get("/domini/" + ID_DOMINIO + "/unitaOperative").with(httpBasic(principaleRistretto, PASSWORD)))
                .andExpect(status().isNotFound());
    }

    /**
     * Regressione: un operatore con solo un grant parziale su una UO (nessun
     * dominio "intero") deve comunque raggiungere il dominio e vedere
     * esclusivamente quella UO, non l'intera lista.
     */
    @Test
    void listRestrictsToGrantedUoForPartialOperator() throws Exception {
        String principaleRistretto = creaOperatoreRistretto();
        UnitaOperativa uoA = uoRepository.findByDominio_IdAndCodUo(dominio.getId(), "UO-A").orElseThrow();
        grantUo(principaleRistretto, dominio.getId(), uoA.getId());

        mvc.perform(get("/domini/" + ID_DOMINIO + "/unitaOperative").with(httpBasic(principaleRistretto, PASSWORD)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.results", hasSize(1)))
                .andExpect(jsonPath("$.results[0].idUnitaOperativa", is("UO-A")));
    }

    @Test
    void getUngrantedUoForPartialOperatorReturns404() throws Exception {
        String principaleRistretto = creaOperatoreRistretto();
        UnitaOperativa uoA = uoRepository.findByDominio_IdAndCodUo(dominio.getId(), "UO-A").orElseThrow();
        grantUo(principaleRistretto, dominio.getId(), uoA.getId());

        mvc.perform(get("/domini/" + ID_DOMINIO + "/unitaOperative/UO-B").with(httpBasic(principaleRistretto, PASSWORD)))
                .andExpect(status().isNotFound());
    }

    @Test
    void getGrantedUoForPartialOperatorSucceeds() throws Exception {
        String principaleRistretto = creaOperatoreRistretto();
        UnitaOperativa uoA = uoRepository.findByDominio_IdAndCodUo(dominio.getId(), "UO-A").orElseThrow();
        grantUo(principaleRistretto, dominio.getId(), uoA.getId());

        mvc.perform(get("/domini/" + ID_DOMINIO + "/unitaOperative/UO-A").with(httpBasic(principaleRistretto, PASSWORD)))
                .andExpect(status().isOk())
    ...
    void listSenzaDirittoAnagraficaCreditoreReturns403() throws Exception {
        ...
        mvc.perform(get("/domini/" + ID_DOMINIO + "/unitaOperative").with(httpBasic(PRINCIPAL_SENZA_DIRITTO, PASSWORD)))
    ...
    void createSenzaDirittoAnagraficaCreditoreReturns403() throws Exception {
        creaOperatoreSenzaDiritto();
        String body = """
                {"idUnitaOperativa":"UO-NEW","ragioneSociale":"UO Nuova","abilitato":true}""";

        mvc.perform(post("/domini/" + ID_DOMINIO + "/unitaOperative")
    ...
    void soloDirittoDiLetturaConsenteListENonCreate() throws Exception {
        ...
        mvc.perform(get("/domini/" + ID_DOMINIO + "/unitaOperative").with(httpBasic(PRINCIPAL_SOLO_LETTURA, PASSWORD)))
                .andExpect(status().isOk());

        String body = """
                {"idUnitaOperativa":"UO-RO","ragioneSociale":"UO RO","abilitato":true}""";
        mvc.perform(post("/domini/" + ID_DOMINIO + "/unitaOperative").with(httpBasic(PRINCIPAL_SOLO_LETTURA, PASSWORD))
                        .contentType(MediaType.APPLICATION_JSON).content(body))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.detail", containsString("Anagrafica Creditore")));
    }
    ...
    void getInesistenteSenzaDirittoReturns403NonRivela404() throws Exception {
        ...
        mvc.perform(get("/domini/99999999999/unitaOperative").with(httpBasic(PRINCIPAL_SENZA_DIRITTO, PASSWORD)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.detail", containsString("Anagrafica Creditore")));
    }
