...
@ActiveProfiles("test")
@Transactional
class AppIoPromemoriaControllerIntegrationTest {

    private static final String PRINCIPAL = "operatore1";
    private static final String PASSWORD = "secret";
    private static final String BASE = "/impostazioni/app-io/template-promemoria";
    ...
    @Test
    void getDefaultReturnsEmptyTemplatesWithoutAllegaPdf() throws Exception {
        grantLettura();
        mvc.perform(get(BASE).with(httpBasic(PRINCIPAL, PASSWORD)))
                .andExpect(status().isOk())
                .andExpect(header().exists("ETag"))
                .andExpect(jsonPath("$.promemoriaAvviso.tipo", is("freemarker")))
                .andExpect(jsonPath("$.promemoriaAvviso.allegaPdf").doesNotExist())
                .andExpect(jsonPath("$.promemoriaRicevuta.allegaPdf").doesNotExist());
    }
    ...
    void putConfigWithIfMatchRoundTrips() throws Exception {
        ...
        mvc.perform(put(BASE).with(httpBasic(PRINCIPAL, PASSWORD))
                ...
                .andExpect(jsonPath("$.promemoriaScadenza.preavviso", is(3)));
    }
    ...
    void patchReplacesSingleBlockAtomically() throws Exception {
        ...
        String etag = currentEtag();
        String patchBody = """
                [{"op":"replace","path":"/promemoriaRicevuta","value":
                    {"tipo":"freemarker","oggetto":"Nuovo oggetto","messaggio":"Nuovo messaggio","soloEseguiti":false}}]""";
        mvc.perform(patch(BASE).with(httpBasic(PRINCIPAL, PASSWORD))
                        .header("If-Match", etag)
                        .contentType(JSON_PATCH).content(patchBody))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.promemoriaRicevuta.oggetto", is("Nuovo oggetto")))
                .andExpect(jsonPath("$.promemoriaRicevuta.soloEseguiti", is(false)))
    ...
    void patchWithoutDirittoReturns403() throws Exception {
        ...
        String etag = currentEtag();
        String patchBody = """
                [{"op":"replace","path":"/promemoriaRicevuta","value":{"tipo":"freemarker"}}]""";
        mvc.perform(patch(BASE).with(httpBasic(PRINCIPAL, PASSWORD))
                        .header("If-Match", etag)
                        .contentType(JSON_PATCH).content(patchBody))
                .andExpect(status().isForbidden());
